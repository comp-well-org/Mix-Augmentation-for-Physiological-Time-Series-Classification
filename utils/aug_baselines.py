...
def cutout(x, ratio=.1):
    ...
    end = min(start + win_len, seq_len - 1)
    output = x.copy()
    output[:, start:end, :] = 0
    return np.squeeze(output, axis=0)
...
def mixup(x, y, device, alpha=0.5):
    """
    Parameters:
    x: input sequence. (torch tensor)
    y: labels of the input sequence. (torch tensor)
    alpha: Parameter for beta distribution in Mixup.
    device: cuda device (torch.device)
    Returns:
    Mixed input, pairs of targets.
    Paper: mixup: Beyond Empirical Risk Minimization https://arxiv.org/abs/1710.09412
    Part of the implementation borrowed from: https://github.com/facebookresearch/mixup-cifar10/blob/main/train.py
    """
    if alpha > 0:
